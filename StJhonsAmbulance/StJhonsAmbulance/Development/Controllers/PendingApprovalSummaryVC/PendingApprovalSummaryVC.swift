//
//  PendingApprovalSummaryVC.swift
//  StJhonsAmbulance
//
...
class PendingApprovalSummaryVC: ENTALDBaseViewController, UIScrollViewDelegate{
    ...
    func reloadControllers(){
        self.eventData = self.dataModel as? PendingApprovalEventsModel
        self.pageController = PagingViewController(viewControllers: [])
//        let eventSummary = EventSummaryScreenVC.loadFromNib()
        let eventDesc = PendingApprovalEventDescriptionVC.loadFromNib()
//        let eventLocation = EventLocationVC.loadFromNib()
        let eventSchedule = PendingApprovalScheduleVC.loadFromNib()
...
extension PendingApprovalSummaryVC: PagingViewControllerDataSource {
