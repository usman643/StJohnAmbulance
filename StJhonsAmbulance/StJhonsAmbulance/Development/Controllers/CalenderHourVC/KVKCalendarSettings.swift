...
extension KVKCalendarSettings where Self: KVKCalendarDataModel {
    ...
    func loadEvents(data:[ScheduleModelThree], dateFormat: String, completion: ([Event]) -> Void) {
        
        let events = data.compactMap({ (item) -> Event in
            let startDate = formatter(date: item.sjavms_start ?? "")
            let endDate = formatter(date: item.sjavms_end ?? "")
            let startTime = timeFormatter(date: startDate, format: dateFormat)
            let endTime = timeFormatter(date: endDate, format: dateFormat)
            
            var event = Event(ID: item.msnfp_participationscheduleid ?? "")
            event.start = startDate
            event.end = endDate
            event.color = Event.Color(UIColor.hexString(hex: "#0000ff"), alpha: 0.3)
            event.isAllDay = false
            event.isContainsFile = false
            
            event.title = TextEvent(timeline: "\(startTime) - \(endTime)\n\(item.sjavms_VolunteerEvent?.msnfp_engagementopportunitytitle ?? "")",
                                    month: "\(item.sjavms_VolunteerEvent?.msnfp_engagementopportunitytitle ?? "") \(startTime)",
                                    list: "\(startTime) - \(endTime) \(item.sjavms_VolunteerEvent?.msnfp_engagementopportunitytitle ?? "")")
...
extension KVKCalendarSettings {
    ...
    func defaultStringDate(_ date:Date)-> String {
        let formatter = DateFormatter()
        formatter.dateFormat = "dd.MM.yyyy"
        return formatter.string(from: date)
    }
    
    func defaultDate(_ date:Date)-> Date {
        onlyDateFormatter.date(from: defaultStringDate(date)) ?? Date()
    }
...
//            return cell
    ...
    func createCalendarStyle(selectedDate:Date = Date()) -> Style {
        var style = Style()
        style.timeline.isHiddenStubEvent = false
        style.startWeekDay = .monday
        style.systemCalendars = ["Calendar1", "Calendar2", "Calendar3"]
        style.event.iconFile = UIImage(systemName: "paperclip")
        style.timeline.scrollLineHourMode = .onlyOnInitForDate(defaultDate(selectedDate))
